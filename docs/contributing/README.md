...
!!! tip
    vLLM requires Python 3.14. vLLM's default [Dockerfile](../../docker/Dockerfile) ships with Python 3.14 and tests in CI are run with Python 3.14.
...
!!! tip "Tips"
    ...
    ```bash
    pre-commit run --hook-stage manual mypy-3.14
    ```
